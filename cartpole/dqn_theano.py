...
class DQN:
    def __init__(self, D, K, hidden_layer_sizes, gamma, max_experiences=10000, min_experiences=100, batch_sz=32):
        ...
        g_updates = [(p, p + v) for p, v, g in zip(self.params, velocities, grads)]
        c_updates = [(c, decay * c + (1 - decay) * g * g) for c, g in zip(caches, grads)]
        v_update = [(v, mu * v - lr * g / T.sqrt(c)) for v, c, g in zip(velocities, caches, grads)]

        updates = g_updates + c_updates + v_update
    ...
    def train(self, target_network):
        # sample a random bacth from buffer, do an iteration of GD
